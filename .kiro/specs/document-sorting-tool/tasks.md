...
- [x] 58. 【需用户决策】确认组件库授权口径
  - **已定档：仅自用、不分发。** 用户于本轮明确选择第一条路径
  - 该口径下 GPLv3 的传染性条款不触发：GPL 的义务由「分发」触发，不分发就没有开源
    或购买商业授权的义务，也不需要更换组件库
  - 随之确定的两条边界，任何后续改动都不得越过：
    - 不把 `dist/` 产物、安装包或源码交付给任何第三方（包括同事、网盘公开链接、
      应用商店、GitHub 公开仓库）
    - 一旦将来要分发，必须先回到本任务重新定档；届时 `app/ui/theme/` 的薄封装层
      （任务 9）是更换组件库的唯一改动面
  - 因此仓库不添加 `LICENSE`：加了反而会声明一个与「不分发」不符的对外授权口径。
    `pyproject.toml` 的 `readme = "README.md"` 仍指向不存在的文件，由任务 59 处理
